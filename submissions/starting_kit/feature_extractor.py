...
# import external_data and merge it
        ext_data = pd.read_csv(os.path.join(path, 'external_data.csv'))

        external_data = ext_data[['DateOfDeparture', 'Departure', 'Arrival',
                                'Distance', 'dep_encod', 'ar_encod',
                                'Events', 'Revenue']]
...
#Reduction and mean encoding airport
        X_encoded.loc[:, 'ATL': 'SFO'] = X_encoded.loc[:, 'ATL': 'SFO']. \
                        mul(X_encoded.loc[:, 'dep_encod'], axis='rows')
        arr = X_encoded.loc[:, 'a_ATL': 'a_SFO']. \
                        mul(- 1 * X_encoded.loc[:, 'ar_encod'], axis='rows')
...
#Encoding date
        ...
        X_encoded['year'] = X_encoded['DateOfDeparture'].dt.year - 2011
        X_encoded['month'] = X_encoded['DateOfDeparture'].dt.month
        X_encoded['day'] = X_encoded['DateOfDeparture'].dt.day
        X_encoded['weekday'] = X_encoded['DateOfDeparture'].dt.weekday
        X_encoded['week'] = X_encoded['DateOfDeparture'].dt.week
        X_encoded['n_days'] = X_encoded['DateOfDeparture']. \
                    apply(lambda date: (date - pd.to_datetime("2011-09-01")).days)
        ...
        X_encoded.drop(['DateOfDeparture', 'month', 'day',
                        'weekday', 'week', 'std_wtd'], axis=1, inplace=True)
...
#Encoding events
        ...
        X_encoded.loc[(X_encoded['WeeksToDeparture'] > 9.524)
                    & (X_encoded['WeeksToDeparture'] <= 11.3), 'WeeksToDeparture'] = 1
        X_encoded.loc[(X_encoded['WeeksToDeparture'] > 11.3)
                    & (X_encoded['WeeksToDeparture'] <= 13.24), 'WeeksToDeparture'] = 2
